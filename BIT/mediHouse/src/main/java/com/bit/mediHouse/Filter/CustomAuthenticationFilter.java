package com.bit.mediHouse.Filter;
//
...
//        String access_token = JWT.create().withSubject(user.getUsername()).withExpiresAt
//                (new Date(System.currentTimeMillis()+10*60*1000))
//                .withIssuer(request.getRequestURI().toString())
//                .withClaim("role", user.getAuthorities().stream().
//                        map(GrantedAuthority::getAuthority).collect(Collectors.toList()))
//                .sign(algorithm);
//        String refresh_token = JWT.create().withSubject(user.getUsername()).withExpiresAt
